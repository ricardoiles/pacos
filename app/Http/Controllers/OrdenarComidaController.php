    ...
    public function store(Request $request){
        ...
        if (empty($datos)) {
            ...
            for ($i = 0; $i < $idplatos; $i++) { 
                ...
                while ($i < $idplatos) {
                    $i;
                    $idres = $request->idres;
                    $platoid = request()->idcomida[$i];
                    $platoprecio = request()->preciocomida[$i];
                    $platocant = request()->cant[$i];
                    $platosubtotal = request()->sub_total[$i];
                    $platosubdesc = request()->sub_desc[$i];
                    $platosubiva = request()->sub_iva[$i];

                    //para insertar en BD
                    DB::table('detalle_reserv')->insert(
                        ['id_Plato' => $platoid, 'precio' => $platoprecio,  'cant' => $platocant, 
                        'Subtotal' => $platosubtotal, 'Sub_desc' => $platosubdesc, 'Sub_Iva' => $platosubiva, 
                        'id_reserva' => $idres]
                    );
        ...
        $reservaciones = DB::table('reservas AS reserv')
                ...
                ->select('rest.id AS idrest', 'rest.nombre AS nombrerest', 'reserv.id AS idreserva', 'reserv.Fecha AS fechareserva', 'reserv.Hora_Ini AS horareserva', 'reserv.Detalle_Reserv AS consincomida', 'reserv.id_Usuario AS iduser', 
                    'reserv.total', 'reserv.total_desc', 'reserv.total_iva', 'mesas.numero', 'mesas.Puestos')
                ->where('reserv.id_Usuario', '=', $iduser)
                ->where(function ($query) use($namepacos, $idres) {
                    $query->where('rest.nombre', '=', $namepacos)
                          ->where('reserv.id', '=', $idres);
                })
                ->get();
